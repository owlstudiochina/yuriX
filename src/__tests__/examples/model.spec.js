...
describe('定义model，并在传入useModel中使用', ()=>{
  ...
  describe('定义高阶model，用于传入model中的依赖', ()=>{
    ...
    it('定义计数器model，可传入自定义的步长step是10，点击累加后值为10', async () =>{
      ...
      const counter  = ({step})=>({set, get})=>{
        return {
          state: {value: 0},
          add: ()=>{
            set({value: get().value+step})
          }
        }
      };
      // When: 传入步长为10，点击累加
      const {result} = renderHook(()=> useModel(()=> counter({step: 10})));
      const {add, getState} = result.current;
  ...
  describe('使用model中的pipe和stopAllPipes,用于方便编写业务逻辑',  ()=>{
    it('用户登录输入账户信息提交，用于登录有点慢，点击中止，停止登录成功', async ()=>{
      ...
      const model = (userService)=> ({set, get}, {pipe, stopAllPipes, isStopError})=>{
        return {
          state: {},
          ...
          useCases: {
            // 登录
            login: (nickname, password)=> pipe(
              concatMap(()=> userService.login(nickname, password))
            ),
            // 停止登录
      ...
      // Given: 定义了登录的model
      const {result} = renderHook(()=> useModel(()=> model(userService)));
      const {
        useCases: {login, stopLogin},
        gets: {isStopLoginErr},
        getState
      } = result.current;
    ...
    it('定义1倍和10倍的计数器，组件使用10倍，点击加后，值应为10', async ()=> {
      ...
      const counter = ({set, get})=>{
        return {
          ...
          add: ()=> {set({value: get().value+ get().step})}
        }
      }

      const counterTenTimes = ({set, get})=>{
        return {
          state: {value: 0, step: 10},
          add: ()=> {set({value: get().value+ get().step})}
        }
      }
      // When: 传入函数选择10倍的model，并且点击加
      const {result} = renderHook(()=> useModel(()=> counterTenTimes))
      const {add, getState} = result.current;
  ...
  describe('使用model中的sync(sync是rxjs的subject)，用于更新状态同步到组件后再进行执行后续业务', ()=>{
    ...
    it('一个计数器，快速点击20次，状态未立即同步到组件，订阅sync等待同步其完成',  async () =>{
      ...
      const {fastClick} = result.current;

      // When: 快速点击并且等待同步完成
      let comValue = 0
      act(()=> {
        fastClick().then(({value})=>{comValue = value})
      })
